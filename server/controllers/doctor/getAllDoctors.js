...
const getAllDoctors = async (req, res) => {
    try {
        const doctorQuery = `select d.education, d.specialities , u.id, concat(u.firstname, ' ', u.lastname) as doctor_name, u.birthdate, u.email, u.phone, a.city, a.house, a.street, a.pincode, h.name as hospital_name, h.is_active as hospital_status from doctors d join users u on d.user_id = u.id and u.is_active = true join address a on u.address_id = a.id join hospitals h on d.hospital_id = h.id;`;
